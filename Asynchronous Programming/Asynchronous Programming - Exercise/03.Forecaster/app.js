function attachEvents() {
    ...
    getBtn.addEventListener('click', async () => {
        try {
            ...
            const { code } = city;
            const currentData =await getForecast('today', code);
            const upcomingData =await getForecast('upcoming', code);
    ...
    async function getForecast(forecastType, code) {
    ...
    function createForecast(forecast, name) {
        ...
        const currentDiv = genElement('div', current, null, 'forecasts');
        genElement('span', currentDiv, weatherSymbols[forecast.condition], 'condition symbol');
        const spanCondition = genElement('span', currentDiv, null, 'condition');
    ...
    function createUpcomingForecast(upcomingForecasts) {
        ...
        const upcomingDiv = genElement('div', upcoming, null, 'forecast-info');
        upcomingForecasts.forEach(forecast => {
            const span = genElement('span', upcomingDiv, null, 'upcoming');
    ...
    function createForecastPart(parent, forecast, name) {
        name
            ? genElement('span', parent, name, 'forecast-data')
            : genElement('span', parent, weatherSymbols[forecast.condition], 'symbol');

        genElement('span', parent,`${forecast.low}${weatherSymbols.Degrees}/${forecast.high}${weatherSymbols.Degrees}`, 'forecast-data');
